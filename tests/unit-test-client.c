...
int main(int argc, char *argv[])
{
    ...
    // NOTE: Increasing the timeout
    rc = modbus_set_response_timeout(ctx, 0, 5000000);
    ...
    rc = modbus_write_bit(ctx, UT_BITS_ADDRESS + UT_BITS_NB, OFF);
    ...
    // NOTE: It can't reply broadcasts!
    ...
    // NOTE: Those special behaviours are not implemented on 28335ModbusSlave, only at libmodbus
    ...
    // NOTE: I didn't understand how it are meant to work
    // /** BAD RESPONSE **/
    ...
    // memset(tab_rp_registers_bad, 0, UT_REGISTERS_NB_SPECIAL * sizeof(uint16_t));
...
int test_server(modbus_t *ctx, int use_backend)
{
    ...
    // NOTE: Not a common function code
    ...
    // NOTE: Too much data - DSP can't handle
